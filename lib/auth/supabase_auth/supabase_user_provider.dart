...
class AuthSupabaseUser extends BaseAuthUser {
  AuthSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> authSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AuthSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
